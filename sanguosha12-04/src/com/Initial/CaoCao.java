...
class HuangZhong extends Character{
	 ...
	 * @param who
	 */
	public void liehun(Player who) {
		if(this.getHp()>who.getHandcard().size()  ||  this.getDisAtt()>who.getDisAtt()) {


			System.out.println("判定烈魂：");
			// 从牌堆中翻出第一张，放入临时集合
			...
			System.out.println("烈魂的判定牌：" + Initial.tmpCheckCard.toString());
			// 花色+数字的判定
			boolean succeed;
			
				
				succeed = Initial.tmpCheckCard.getColor().equals("红桃")
						&& Initial.tmpCheckCard.getColor().equals("方块");
				if (succeed == true) {
					System.out.println("判定生效！" + who.toString() + "此杀不可避免！");
					who.setHp(who.getHp() - 1);					
					who.printHp();
				} else {
					System.out.println("判定无效");					
				}
